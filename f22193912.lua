...
function library:New(name) 
	...
	do 
		...
		TextLabel.BorderColor3 = COL3RGB(148,0,211) 
		TextLabel.Position = UDIM2(0, 3, 0.40003, 0) 
		TextLabel.Size = UDIM2(0, 180, 0, 15) 
		TextLabel.ZIndex = 2 
		TextLabel.Font = Enum.Font.SourceSansSemibold 
		TextLabel.Text = "keybinds" 
		TextLabel.TextColor3 = COL3RGB(148,0,211) 
...
local gui = library:New("FloppaWarre$$$") 
...
antiaim:Element("Dropdown", "pitch", {options = {"none", "up", "down", "zero", "180", "Bodyarm", "custom1", "random"}}) 
...
antiaim:Element("Slider", "high pos", {min = -3, max = 20, default = 3})
...
fakelag:Element("ToggleKeybind", "enabled") 
...
exploits:Element("ToggleKeybind", "resolver right") 
exploits:Element("ToggleKeybind", "resolver left") 
...
world:Element("Slider", "sound volume", {min = 1, max = 20, default = 3}) 
world:Element("Dropdown", "skybox", {options = {"none", "nebula", "vaporwave", "clouds", "Pink Daylight"}}, function(tbl) 
...
movement:Element("Slider", "speed", {min = 1, max = 500, default = 30}) 
...
chat:Element("Toggle", "chat spam", nil, function(tbl) 
	if tbl.Toggle then 
		while values.misc.chat["chat spam"].Toggle do 
			game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "FloppaWarre$$$ Winnig omg you die stop try inject private jjsploit :thinking: :thinking: :thinking: :thinking: :thinking: :thinking: :thinking: ahahh just get flopawarre$$ " or "noob Stop crying :thinking: aahha you noob why you not use jjsploit lol ahhaaaaha use :thinking:", false, "Innocent", false, true) 
			wait(values.misc.chat["speed (ms)"].Slider/1000)  
		end 
	end 
end) 
chat:Element("Dropdown", "type", {options = {"FloppaWarre$$$", "noob"}}) 
chat:Element("Slider", "speed (ms)", {min = 15, max = 1000, default = 500}) 
...
Dance.AnimationId = "rbxassetid://7202898984"
Dance.AnimationId = "rbxassetid://3823158750"
local LoadedAnim 
...
animations:Element("Dropdown", "animation", {options = {"floss", "default", "Godlike", "dolphin", "monkey"}}, function(tbl) 
	Dance.AnimationId = tbl.Dropdown == "floss" and "rbxassetid://5917459365" or tbl.Dropdown == "default" and "rbxassetid://3732699835" or tbl.Dropdown == "Godlike" and "rbxassetid://3823158750" or tbl.Dropdown == "dolphin" and "rbxassetid://5938365243" or tbl.Dropdown == "monkey" and "rbxassetid://3716636630" 
...
RunService.RenderStepped:Connect(function(step) 
	...
	if PlayerIsAlive then 
		...
		if values.rage.fakelag["ping spike"].Toggle and values.rage.fakelag["ping spike"].Active then 
			for count = 1, 20  do 
				game:GetService("ReplicatedStorage").Events.RemoteEvent:FireServer({[1] = "createparticle", [2] = "bullethole", [3] = LocalPlayer.Character.Head, [4] = Vec3(4,4,0)})  
			end 
		end 
		...
		if workspace:FindFirstChild("Map") and Client.gun ~= "none" and Client.gun.Name ~= "C4" then 
			if values.rage.aimbot.enabled.Toggle then 
				local Origin = values.rage.aimbot.origin.Dropdown == "character" and LocalPlayer.Character.LowerTorso.Position + Vec3(3, 4, 0) or CamCFrame.p 
				...
				for _,Player in pairs(Players:GetPlayers()) do 
					...
					if values.rage.exploits["resolver right"].Toggle and values.rage.exploits["resolver right"].Active then
						Player.Character.UpperTorso.Waist.C0 = CFAngles(0, 3, -1.5)
				    end
					if values.rage.exploits["resolver left"].Toggle and values.rage.exploits["resolver left"].Active then
						Player.Character.UpperTorso.Waist.C0 = CFAngles(0, 3, 1.5)
					end
		...
		if values.rage.angles.enabled.Toggle and not DisableAA then 
	        ...
	        local Pitch = values.rage.angles["pitch"].Dropdown == "none" and CamLook.Y or values.rage.angles["pitch"].Dropdown == "up" and 1 or values.rage.angles["pitch"].Dropdown == "down" and -1 or values.rage.angles["pitch"].Dropdown == "zero" and 0 or values.rage.angles["pitch"].Dropdown == "Bodyarm" and -15 or values.rage.angles["pitch"].Dropdown == "custom1" and 5 or values.rage.angles["pitch"].Dropdown == "random" and RANDOM(-100, 100)/10 or 2.5
			if values.rage.angles["extend pitch"].Toggle and (values.rage.angles["pitch"].Dropdown == "up" or values.rage.angles["pitch"].Dropdown == "down" or values.rage.angles["pitch"].Dropdown == "Bodyarm") then 
				Pitch = (Pitch*2)/0.4 
			end 
	...
	if method == "FindPartOnRayWithIgnoreList" and args[2][1] == workspace.Debris then 
		if not checkcaller() or Filter then 
			...
			if RageTarget ~= nil then 
				local Origin = values.rage.aimbot.origin.Dropdown == "character" and LocalPlayer.Character.LowerTorso.Position + Vec3(3, 4, 0) or Camera.CFrame.p 
	...
	if method == "FireServer" and self.Name == "HitPart" then 
		...
		if values.visuals.world["bullet tracers"].Toggle then 
			coroutine.wrap(function() 
				...
				library:Tween(beam, TweenInfo.new(1.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Transparency = 0}) 
				wait(1.5) 
				beam:Destroy() 
			end)() 
		end 
...
LocalPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(current) 
	...
	if values.misc.chat["kill say"].Toggle then 
		game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat["message"].Text ~= "" and values.misc.chat["message"].Text or "Get Good Buy shootware$$$", false, "Innocent", false, true) 
	end 
end) 
...
Camera.ChildAdded:Connect(function(obj) 
	if TBLFIND(values.misc.client["gun modifiers"].Jumbobox, "ammo") then 
		Client.ammocount = 900000000
		Client.primarystored = 900000000 
		Client.ammocount2 = 900000000 
		Client.secondarystored = 900000000 
	end 
	RunService.RenderStepped:Wait() 
